...
// A simple thread-safe queue for Transactions
template<typename T>
...
public:
    ...
    // Pops one element; returns false if queue empty and finished()
    bool pop(T& out) {
...
void writerWorker(const std::string& outFile) {
    ...
    while (txQueue.pop(tx)) {
        ...
        writer.Key("time_since_last_transaction");
                                      writer.String(tx.time_since_last_transaction.c_str());
        writer.Key("spending_deviation_score");
                                      writer.Double(tx.spending_deviation_score);
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::fprintf(stderr, "Usage: %s <input.csv> <output.json>\n", argv[0]);
        return 1;
    }

    // Launch one parser thread (could be >1 if you split the file)
    std::thread parser(parserWorker,  std::string(argv[1]));
    // Launch writer thread
    std::thread writer(writerWorker, std::string(argv[2]));
